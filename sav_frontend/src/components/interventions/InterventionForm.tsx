...
const InterventionForm: React.FC<Props> = ({ open, intervention, onClose, onSave }) => {
  ...
  const handleChange = (k: keyof Intervention, v: any) => {
    if (k === 'reclamationId' || k === 'technicienId' || k === 'coutPieces' || k === 'coutMainOeuvre') {
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent>
        <Box sx={{ mt: 0.5, display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' } }}>
          ...
          <Box>
            <TextField 
              id="interv-form-gratuite" 
              label="Gratuite" 
              select 
              value={form.estGratuite ? 'oui' : 'non'} 
              fullWidth 
              onChange={e => handleChange('estGratuite', e.target.value === 'oui')}
            >
      ...
      <DialogActions>
        ...
        <Button variant="contained" onClick={() => onSave(form)} disabled={disableSave}>
          Enregistrer
        </Button>
      </DialogActions>
    </Dialog>
