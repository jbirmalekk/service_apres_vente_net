...
export const clientService = {
  ...
  getByEmail: async (email: string): Promise<Client> => {
    const res = await fetch(`${BASE}/email/${encodeURIComponent(email)}`, { headers: getAuthHeaders() });
    return handleResponse(res);
  },
  search: async (term: string): Promise<Client[]> => {
    const res = await fetch(`${BASE}/search/${encodeURIComponent(term)}`, { headers: getAuthHeaders() });
    return handleResponse(res);
  },
  advancedSearch: async (params: Record<string, any>): Promise<Client[]> => {
    ...
    const res = await fetch(`${BASE}/advanced?${q.toString()}`, { headers: getAuthHeaders() });
    return handleResponse(res);
  },
  create: async (client: Partial<Client>): Promise<Client> => {
    const res = await fetch(`${BASE}`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...getAuthHeaders() }, body: JSON.stringify(client) });
    return handleResponse(res);
  },
  update: async (id: number, client: Partial<Client>): Promise<Client> => {
    const res = await fetch(`${BASE}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', ...getAuthHeaders() }, body: JSON.stringify(client) });
    return handleResponse(res);
  },
  ...
  getClientsAvecReclamations: async (): Promise<Client[]> => {
    const res = await fetch(`${BASE}/avec-reclamations`, { headers: getAuthHeaders() });
    return handleResponse(res);
  },
  getReclamationsForClient: async (id: number) => {
    const res = await fetch(`${BASE}/${id}/reclamations`, { headers: getAuthHeaders() });
    return handleResponse(res);
  }
};
