...
export const interventionService = {
  ...
  byReclamation: async (reclamationId: number): Promise<Intervention[]> => {
    const res = await fetch(`${BASE}/reclamation/${reclamationId}`, { headers: getAuthHeaders() });
    return handleResponse(res);
  },
  byTechnicien: async (technicienId: number): Promise<Intervention[]> => {
    const res = await fetch(`${BASE}/technicien/${technicienId}`, { headers: getAuthHeaders() });
    return handleResponse(res);
  },
  byStatut: async (statut: string): Promise<Intervention[]> => {
    const res = await fetch(`${BASE}/statut/${encodeURIComponent(statut)}`, { headers: getAuthHeaders() });
    return handleResponse(res);
  },
